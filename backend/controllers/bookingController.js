...
//We also need to check for the bboking as a user has already enrolled in a course or not
    ...
    try {
        ...
        if (!userId) {
            return res.status(200).json({ 
                success: true, 
                enrolled: !!paid, // if not authenticated, treat as not enrolled
                booking: null
             });
            } 
        ...
        if (!courseId) return res.status(400).json({ 
            ...
            booking.paidAt; // <-- also consider presence of paidAt as confirmation
