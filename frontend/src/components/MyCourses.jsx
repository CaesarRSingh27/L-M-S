...
const MyCourses = () => {
    ...
    const fetchMyCourses = async () => {
      ...
      try {
        ...
        // ✅ FIX 1: removed /my-rating fetch — endpoint returns 404
  ...
  const submitRatingToServer = async (courseId, ratingValue) => {
    try {
      ...
      const avg =
        data.avgRating ?? data.course?.avgRating ?? null;
      const total =
        data.totalRatings ?? data.course?.ratingCount ?? null;
  ...
  const handleSetRating = async (e, courseId, rating) => {
    e.stopPropagation();
    // ✅ FIX 2: was const { isSignedIn: signed } = {isSignedIn} which is always undefined
    if (!isSignedIn) {
  ...
  return (
    // ✅ FIX 3: wrapped in Fragment so <style> is a sibling, not outside return
    <>
      <div className={myCoursesStyles.pageContainer}>
        <div className={myCoursesStyles.mainContainer}>
          <h1 className={myCoursesStyles.header}>My Courses</h1>
            ...
            {courses.map((course, index) => (
              ...
              >
                <div className={myCoursesStyles.imageContainer}>
                  {/* ✅ FIX 4: was using courseCard class on img, also guard empty image */}
                  {course.image ? (
                    <img
                      src={course.image}
                      alt={course.name}
      ...
      {/* ✅ FIX 3: removed jsx attribute, moved inside Fragment */}
