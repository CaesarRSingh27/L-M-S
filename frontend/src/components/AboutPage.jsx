                    ...
                    <div className={aboutUsStyles.valuesGrid}>
                        {values.map((value, index) => (
                            <div key={index} className={aboutUsStyles.valueCard}>
                                ...
                                <h3 className={aboutUsStyles.valueCardTitle}>
                                    {value.title}
                                </h3>
                                ...
                                <ul className={aboutUsStyles.valueFeatures}>
                                    {value.features.map((feature, i) => (
                                        <li key={i} className={aboutUsStyles.valueFeatureItem}>
                                            <div className={`${aboutUsStyles.valueFeatureDot} ${value.color}`}></div>
                                            {feature}
                                        </li>
                                    ))}
                                </ul>

                                <div className={`${aboutUsStyles.valueUnderline} ${value.color}`}></div>
                            </div>
                        ))}
                    </div>
                </div>
                ...
                <div className={aboutUsStyles.sectionGrid}>
                    ...
                    <div className={aboutUsStyles.testimonialsGrid}>
                        {testimonials.map((t, i) => (
                            <div key={i} className={aboutUsStyles.testimonialCard}>
                                <div className={aboutUsStyles.testimonialStars}>
                                    {[...Array(t.rating)].map((_, j) => (
                                        <Star key={j} className={aboutUsStyles.testimonialStar} />
                                    ))}
                                </div>
                                <p>"{t.text}"</p>
                            </div>
...
<section className={aboutUsStyles.teamSection}>
  <div className={aboutUsStyles.sectionGrid}>
    ...
    {/* ✅ FIXED GRID */}
    <div className={`${aboutUsStyles.teamGrid} grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-12 justify-items-center`}>

      {teamMembers.map((member, index) => (
        <div key={index} className={`${aboutUsStyles.teamMember} max-w-xs text-center`}>
          ...
          {/* NAME */}
          <h3 className={`${aboutUsStyles.teamName} font-semibold text-lg mt-4`}>
            {member.name}
          </h3>

          {/* ROLE */}
          <div className={`${aboutUsStyles.teamRole} text-blue-600 italic text-sm`}>
            {member.role}
          </div>

          {/* BIO */}
          <p className={`${aboutUsStyles.teamBio} text-gray-600 text-sm mt-2`}>
            {member.bio}
          </p>
      ...
      <section className={aboutUsStyles.ctaSection}>
        ...
        <div className={aboutUsStyles.ctaContent}>
            ...
            <p className={aboutUsStyles.ctaDescription}>
                Join millions of learners who have tranformed their lives with 
