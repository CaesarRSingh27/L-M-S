...
const UserIcon = () => (
  <User className={coursePageStyles.teacherContainer}/>
);
...
// Fetch public courses
    ...
    fetch(`${API_BASE}/api/courses/public`)
      ...
      .then(async (json) => {
        ...
        const mapped = regular.map((c) => ({
          ...
          category: c.category || "",
          // ✅ FIX 1: guard empty string image so src="" warning disappears
          image: c.image && c.image.trim() !== "" ? c.image : null,
          isFree:
            c.pricingType === "free" ||
            !c.price ||
        ...
        // ✅ FIX 2: removed /my-rating parallel fetch — endpoint returns 404
        // ratings persist via localStorage instead
      })
      .catch((err) => {
  ...
  const submitRatingToServer = async(courseId, ratingValue) => {
    try {
        // ✅ FIX 3: was missing quotes — application/json is not a variable
        ...
        const res = await fetch(
          `${API_BASE}/api/courses/${encodeURIComponent(courseId)}/rate`,
          {method: "POST", 
            headers,
            credentials: "include",
    ...
    <div className={coursePageStyles.pageContainer}>
      ...
      <div className={coursePageStyles.headerContainer}>
        ...
        <div className={coursePageStyles.searchContainer}>
          ...
          <div
            className={coursePageStyles.searchGradient}
                  ...
                  <div className={coursePageStyles.courseCardInner}>
                    <div className={coursePageStyles.courseCardContent}>
                      ...
                      <div className={coursePageStyles.courseImageContainer}>
                        {/* ✅ FIX 1: conditionally render img only if image exists */}
                        {course.image ? (
                          <img
                            src={course.image}
                            alt={course.name}
      ...
      {/* ✅ FIX 4: removed duplicate <style> tag that was here */}
    </div>
